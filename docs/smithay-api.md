...
Quick reference for key smithay APIs used in hordora. See the source at
...
let state = CursorShapeManagerState::new::<Hordora>(&display_handle);
delegate_cursor_shape!(Hordora);
// Also need: impl TabletSeatHandler for Hordora {}
```
...
- **Must call `on_commit_buffer_handler::<Hordora>(surface)`** in `CompositorHandler::commit()` — NOT done by `delegate_compositor!`. Without it, `RendererSurfaceState` is never populated, `surface_view` stays None, `bbox_from_surface_tree()` returns 0x0, windows invisible.
- **Must call `output.create_global::<Hordora>(&display_handle)`** — `space.map_output()` is internal only; clients need a `wl_output` global to see monitors.
...
- **Backend on state** — winit backend stored as `Option<WinitGraphicsBackend<GlesRenderer>>` on Hordora. Timer closure uses take/put pattern to split borrows. Required for DmabufHandler to access renderer.
