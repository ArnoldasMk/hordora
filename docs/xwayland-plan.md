# XWayland Support for hordora

## Context

hordora is a pure-Wayland compositor. Many Linux apps (Steam, Wine/Proton games, older GTK2/Qt4 apps, JetBrains IDEs) require X11. Adding XWayland support lets these apps run on the infinite canvas alongside native Wayland windows.

smithay 0.7.0's `Window` type already wraps both Wayland and X11 via `WindowSurface` enum — but hordora currently calls `.toplevel().unwrap()` **78 times across 15 files**, which will panic for X11 windows. The biggest part of this work is making the codebase window-type-agnostic.
...
Initialize `XWaylandShellState::new::<Self>(&dh)` in `Hordora::new()`.
...
Add `pub mod xwayland;` to `src/handlers/mod.rs`. Add `delegate_xwayland_shell!(Hordora);`.
...
| `maximize/minimize` | Ignore (no maximize/minimize in hordora) |
