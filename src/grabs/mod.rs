...
/// Assumes `self.start_data: GrabStartData<Hordora>`.
macro_rules! forward_pointer_grab_methods {
    () => {
        fn relative_motion(
            &mut self,
            data: &mut crate::state::Hordora,
            handle: &mut smithay::input::pointer::PointerInnerHandle<'_, crate::state::Hordora>,
            focus: Option<(
                <crate::state::Hordora as smithay::input::SeatHandler>::PointerFocus,
        ...
        fn start_data(&self) -> &smithay::input::pointer::GrabStartData<crate::state::Hordora> {
            &self.start_data
        }
    };
}
