...
use hordora::canvas::{CanvasPos, canvas_to_screen};
use hordora::snap::{SnapParams, SnapState, update_axis};
use crate::state::{Hordora, output_state, output_logical_size};
...
pub struct MoveSurfaceGrab {
    pub start_data: GrabStartData<Hordora>,
    pub window: Window,
...
impl MoveSurfaceGrab {
    pub fn new(
        start_data: GrabStartData<Hordora>,
        window: Window,
...
impl PointerGrab<Hordora> for MoveSurfaceGrab {
    fn motion(
        &mut self,
        data: &mut Hordora,
        handle: &mut PointerInnerHandle<'_, Hordora>,
        _focus: Option<(<Hordora as SeatHandler>::PointerFocus, Point<f64, Logical>)>,
        event: &MotionEvent,
    ) {
    ...
    fn unset(&mut self, _data: &mut Hordora) {
        output_state(&self.output).edge_pan_velocity = None;
    }
