...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if std::env::args().any(|a| a == "--version" || a == "-V") {
        println!("hordora {}", env!("CARGO_PKG_VERSION"));
        return Ok(());
    }

    // --check-config: validate config and exit
    if std::env::args().any(|a| a == "--check-config") {
        let _config = hordora::config::Config::load();
    ...
    let mut event_loop: smithay::reexports::calloop::EventLoop<Hordora> =
        smithay::reexports::calloop::EventLoop::try_new()?;

    // Create Wayland display
    let display =
        smithay::reexports::wayland_server::Display::<Hordora>::new()?;

    // Build compositor state
    let mut data = Hordora::new(
        display.handle(),
        event_loop.handle(),
    ...
    event_loop.handle().insert_source(display_source, |_, display, data: &mut Hordora| {
    ...
    unsafe { std::env::set_var("XDG_CURRENT_DESKTOP", "hordora") };
    ...
    unsafe { std::env::set_var("XDG_SESSION_DESKTOP", "hordora") };
    ...
    event_loop
        .handle()
        .insert_source(listening_socket, |stream, _, data: &mut Hordora| {
    ...
    // Config file watcher: poll mtime every 500ms
    {
        ...
        event_loop.handle().insert_source(timer, move |_, _, data: &mut Hordora| {
