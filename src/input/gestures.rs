...
fn set_accel_profile(
    ...
    profile: hordora::config::AccelProfile,
) {
    use hordora::config::AccelProfile;
            ...
            if let Some(entry) = dt_entry {
                ...
                match entry {
                    ...
                    GestureConfigEntry::Continuous(ContinuousAction::ResizeWindow) => {
                        if let Some((window, _)) = self.window_under(pos)
                            .filter(|(w, _)| {
                                !w.wl_surface().as_ref().and_then(|s| hordora::config::applied_rule(s))
            ...
            GestureState::SwipeResize {
                ...
                *cumulative += clamped_delta;
                    ...
                    && let Some(self_surface) = window.wl_surface().map(|s| s.into_owned())
                {
                    ...
                    let mut others: Vec<hordora::snap::SnapRect> = Vec::new();
                    for w in self.space.elements() {
                        let Some(surface) = w.wl_surface() else { continue };
                        if *surface == self_surface { continue }
                        if hordora::config::applied_rule(&surface).is_some_and(|r| r.widget) { continue }
