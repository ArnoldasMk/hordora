#!/usr/bin/env bash
#
# Torture test for hordora — hammers the compositor to surface resource leaks.
# Run this INSIDE a hordora session. Monitors fd count and memory throughout.
#
# Usage:
...
compositor_pid() {
    if [[ -z "$PID" ]]; then
        PID=$(pidof hordora 2>/dev/null || true)
        if [[ -z "$PID" ]]; then
            echo "ERROR: hordora not running" >&2
            exit 1
        fi
    fi
    echo "$PID"
